...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    log::info!("WAV format: {} Hz, {} channel(s), {} bits per sample",
    ...
    let mono_samples: Vec<i16> = match spec.channels {
        ...
        _ => {
            eprintln!("Error: {} channels not supported (only mono and stereo)", spec.channels);
            std::process::exit(1);
        }
    };
    ...
    log::info!("Playing {} ({} Hz, {:.2}s)...", wav_path, spec.sample_rate, duration_secs);

    // Create AudioOut and send samples
    let mut audio_out = AudioOut::new(None, spec.sample_rate);
    audio_out.send(&mono_samples).await?;
