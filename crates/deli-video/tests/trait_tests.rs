use deli_video::{CameraError, VideoData, VideoFrame};
...
async fn test_video_frame_rgb_variant() {
    ...
    let frame = VideoFrame { data: VideoData::Rgb(tensor), width: 2, height: 2 };
    match frame.data {
...
async fn test_video_frame_jpeg_variant() {
    let frame = VideoFrame { data: VideoData::Jpeg(vec![0xFF, 0xD8, 0xFF, 0xE0]), width: 640, height: 480 };
    match frame.data {
        VideoData::Jpeg(data) => {
