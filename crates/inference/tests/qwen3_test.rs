...
async fn test_forward_empty_prompt_validation() {
    let model_path = Path::new("../../data/qwen3/qwen3-8b-q4_k_m.gguf");
    let tokenizer_path = Path::new("../../data/qwen3/tokenizer.json");

    if !model_path.exists() || !tokenizer_path.exists() {
        println!("Skipping test - model files not found at data/qwen3/");
        return;
    }
