...
async fn test_with_real_model() {
    let model_path = Path::new("data/whisper/tiny.en/model.safetensors");
    let tokenizer_path = Path::new("data/whisper/tiny.en/tokenizer.json");
    let config_path = Path::new("data/whisper/tiny.en/config.json");

    if !model_path.exists() || !tokenizer_path.exists() || !config_path.exists() {
        println!("Skipping test - model files not found at data/whisper/tiny.en/");
        return;
    }
