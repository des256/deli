...
/// Quantized Phi3 language model for text generation.
///
/// Phi3 uses the Phi3 architecture from candle-transformers with quantized weights
...
/// (one at a time per Phi3 instance).
#[derive(Clone)]
pub struct Phi3 {
...
impl Phi3 {
    /// Create a new Phi3 model from GGUF file and tokenizer.
    ///
    /// Validates the tokenizer contains the required `<|endoftext|>` EOS token at
    /// construction time. The optional `<|end|>` end-of-turn token is looked up
    /// but not required.
    ///
    /// # Arguments
    ...
    ///
    /// # Returns
    /// A new Phi3 instance ready for text generation
    ///
    /// # Errors
    /// Returns error if:
    ...
    pub fn new(
        ...
        // Load model weights (from_gguf takes 4 args: use_flash_attn, ct, reader, device)
        // Pass false for use_flash_attn
    ...
    ///
    /// Generation stops at `<|endoftext|>` (primary EOS token) or `<|end|>`
