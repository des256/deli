...
pub struct Gemma3Handle<T: Clone + Send + 'static> {
    input_tx: std_mpsc::Sender<Stamped<LlmInput<T>>>,
    epoch: Epoch,
}

pub struct Gemma3Listener<T: Clone + Send + 'static> {
...
pub fn create<T: Clone + Send + 'static>(
    ...
    epoch: Epoch,
) -> Result<(Gemma3Handle<T>, Gemma3Listener<T>), InferError> {
...
impl<T: Clone + Send + 'static> Gemma3Listener<T> {
