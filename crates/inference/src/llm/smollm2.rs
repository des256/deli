...
/// Quantized SmolLM2 1.7B language model for text generation.
///
/// SmolLM2 uses the Llama architecture from candle-transformers with quantized weights
...
/// (one at a time per SmolLm2 instance).
#[derive(Clone)]
pub struct Smollm2 {
...
impl Smollm2 {
    ...
    ///
    /// # Returns
    /// A new SmolLm2 instance ready for text generation
    ///
    /// # Errors
    /// Returns error if:
    ...
    pub fn new(
        ...
        // SmolLM2 uses Llama architecture
    ...
    pub async fn forward(&self, prompt: &str, sample_len: usize) -> Result<String, InferError> {
        ...
        tokio::task::spawn_blocking(move || -> Result<String, InferError> {
            ...
            // SmolLM2's KV cache auto-resets when index_pos == 0, no explicit clear needed
