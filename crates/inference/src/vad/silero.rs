...
///
/// Targets Silero VAD **v5 only**. The model must have:
/// - Inputs: `input` (audio `[1, N]`), `state` (`[2, 1, 128]`), `sr` (sample rate `[1]`)
/// - Outputs: `output` (probability `[1, 1]`), `stateN` (updated state `[2, 1, 128]`)
///
/// Silero VAD v4 models (with separate h/c state tensors) are **not** supported.
///
/// Processes audio frames (512 samples at 16kHz) and returns speech probability.
/// Maintains internal state across frames for temporal context.
...
impl SileroVad {
    ...
    pub fn new(session: Session) -> Result<Self> {
        ...
        let state = onnx::Value::zeros::<f32>(&[2, 1, 128]).map_err(|e| {
    ...
    pub fn process(&mut self, audio_frame: &[f32]) -> Result<f32> {
        ...
        let sr_tensor = onnx::Value::from_slice::<i64>(&[1], &[self.sample_rate])
        ...
        self.context.copy_from_slice(&audio_frame[FRAME_SIZE - CONTEXT_SIZE..]);
    ...
    pub fn reset(&mut self) -> Result<()> {
        ...
        self.state = onnx::Value::zeros::<f32>(&[2, 1, 128]).map_err(|e| {
            InferError::Runtime(format!("Failed to reset VAD state: {}", e))
        })?;
