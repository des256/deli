...
impl deli_codec::Codec for OversizedPayload {
    fn encode(&self, buf: &mut Vec<u8>) {
        buf.resize(self.0, 0);
    }

    fn decode(_buf: &[u8], _pos: &mut usize) -> Result<Self, deli_codec::DecodeError> {
        Err(deli_codec::DecodeError::UnexpectedEof)
    }
}
