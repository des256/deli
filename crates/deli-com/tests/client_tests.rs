...
async fn test_receiver_multiple_messages() {
    ...
    assert_eq!(timeout(Duration::from_secs(5), receiver.next()).await.unwrap().unwrap().unwrap(), 10);
    assert_eq!(timeout(Duration::from_secs(5), receiver.next()).await.unwrap().unwrap().unwrap(), 20);
    assert_eq!(timeout(Duration::from_secs(5), receiver.next()).await.unwrap().unwrap().unwrap(), 30);
}
...
async fn test_receiver_connection_closed() {
    ...
    // next() should return Some(Err(ConnectionClosed)) because server closed the connection
    let result = timeout(Duration::from_secs(5), receiver.next())
        .await
        .expect("recv timed out");
    match result {
        Some(Err(ComError::ConnectionClosed)) => {} // Expected
        other => panic!("Expected Some(Err(ConnectionClosed)), got {:?}", other),
    }
}
