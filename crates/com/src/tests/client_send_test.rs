use {com::{Client, Server}, futures_util::{SinkExt, StreamExt}, tokio::time::{Duration, sleep, timeout}};
...
async fn test_client_send_to_server() {
    ...
    // Connect client
    let mut client = Client::<u32>::connect(addr).await.expect("connect failed");
