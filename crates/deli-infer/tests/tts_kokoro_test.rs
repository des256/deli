...
fn test_implements_sink_and_stream() {
    ...
    fn assert_stream<T: Stream<Item = deli_infer::Result<AudioSample>>>() {}
    assert_sink::<Kokoro>();
...
async fn test_kokoro_integration() {
    ...
    let sample = kokoro.next().await.expect("stream should yield an item").unwrap();
    ...
    assert!(kokoro.next().await.is_none(), "stream should end after close");
    ...
    for s in &tensor.data {
        file.write_all(&s.to_le_bytes()).expect("Failed to write sample");
    }
...
async fn test_kokoro_multiple_utterances() {
    ...
    for text in &texts {
        let sample = kokoro.next().await.expect("stream should yield an item").unwrap();
        let AudioData::Pcm(tensor) = sample.data;
        assert!(!tensor.data.is_empty(), "Audio should not be empty for: {}", text);
        println!("{}: {} samples", text, tensor.data.len());
    }

    assert!(kokoro.next().await.is_none(), "stream should end after all items");
}
...
async fn test_kokoro_long_text() {
    ...
    let sample = kokoro.next().await.expect("stream should yield an item").unwrap();
    let AudioData::Pcm(tensor) = sample.data;
    assert!(!tensor.data.is_empty(), "Moderate text should produce audio");
