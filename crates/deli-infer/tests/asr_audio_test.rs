use deli_infer::asr::config::N_SAMPLES;
use deli_infer::asr::{load_mel_filters, pcm_to_mel, Config};
...
fn test_load_mel_filters() {
    ...
    assert!(non_zero_count > 0, "Mel filters should contain non-zero values");
}
...
fn test_fft_with_sine_wave() {
    ...
    assert!(variance > 0.0, "Mel spectrogram should contain energy for sine wave (variance = {variance})");
}
