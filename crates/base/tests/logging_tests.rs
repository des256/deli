use base::logging::{FileLogger, StdoutLogger, init_file_logger, init_stdout_logger};
use log::Log;
use std::fs;
...
fn test_init_file_logger_invalid_dir_returns_error() {
    ...
    assert!(
        result.is_err(),
        "init_file_logger with invalid path should return Err"
    );
}
